...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.postimg.cc/zDMH1G4j/bubu-funny-cute.gif';
...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("⎯͢⎯⃝উৃ্ঁষ্ঠাৃ্ঁ খেৃ্ঁয়েৃ্ঁ কৃ্ঁইৃ্ঁ চৃ্ঁলেৃ্ঁ আৃ্ঁসৃ্ঁলাৃ্ঁমৃ্ঁ আৃ্ঁমিৃ্ঁ 🤡🐍⎯͢⎯⃝😁!", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
.catch(error => {
    ...
    try {
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        ...
        var ok = [
          'https://i.imgur.com/2QAOAvm.jpeg',
          'https://i.imgur.com/bI2Cf22.jpeg'
        ]
